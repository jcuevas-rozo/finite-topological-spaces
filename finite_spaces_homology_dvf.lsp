...
(provide "Finite_Spaces_Homology_DVF")

;;
;;  Computing homology of h-regular posets using discrete vector fields
;;
      ...
      (let ((ptl1 (baselig mtrx1 i))
        ...
        (do () ((and (eq pl1 ptl1) (eq pl2 ptl2)))
          (let ((ic1 (icol pl1))
                (ic2 (icol pl2)))
            (cond
             ((< ic1 ic2) (progn (maj-terme (chercher-hor plr ic1) (basecol rslt ic2) (- (val pl2)))
                            (setf pl2 (left pl2))))
             ((< ic2 ic1) (progn (maj-terme (chercher-hor plr ic2) (basecol rslt ic1) (val pl1))
                            (setf pl1 (left pl1))))
             (T (progn (maj-terme (chercher-hor plr ic1) (basecol rslt ic1) (- (val pl1) (val pl2)))
                  (setf pl2 (left pl2)
                        pl1 (left pl1)))))))))))
      ...
      (do ((pl01 (baselig 4blocks index))
        ...
        (if (> (icol p1) size-d21)
          ...
          (unless (or (zerop (val p1)) (eq (icol p1) index))
            (0column-op 4blocks (- (val p1)) index (icol p1))))))
    ...
    (do ((il (1+ size-d21) (1+ il)))
      ...
      (do ((pl01 (baselig 4blocks il))
        ...
        (let ((p31 (left (baselig d31-mtrx (- il size-d21))))
              ...
              (ic (icol p1)))
          (if (> ic size-d21)
              (progn (maj-terme (chercher-hor p33 (- ic size-d21)) (basecol d33-mtrx (- ic size-d21)) (val p1))
                (setf p33 (left p33)))
            (progn (maj-terme (chercher-hor p31 ic) (basecol d31-mtrx ic) (val p1))
              (setf p31 (left p31)))))))
    (mtrx-substract d33-mtrx (mtrx-prdc d31-mtrx d23-mtrx))))
...
(DEFMETHOD H-REGULAR-DIF-DVF ((finspace finite-space) &key targets sources)
  (let* ((diferentials '())
         ...
         (TSC-decomp (decomposition (heights finspace) elements-heights targets sources)))
    (declare (type list diferentials))
    (declare (type matrice old-differential))
    (push (creer-matrice 0 (length (third (car TSC-decomp )))) diferentials)
    (do ((dim_Cn-2 NIL (length (car heights)))
      ...
      (let* ((Cn-1 (car heights)) (TSC_n-1 (car TSC))
        ...
        (if dim_Cn-2
            (setf old-differential (let ((dn-1 (car diferentials)))
        ...
        (push (D33-D31D21D23
               ...
               (length (first TSC_n))) diferentials)))
    (return-from H-REGULAR-DIF-DVF (reverse diferentials))))
