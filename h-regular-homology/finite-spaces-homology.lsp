...
(DEFMETHOD H-REGULAR-DIF ((finspace finite-space))
  (let ((differentials '())
    ...
    (push (creer-matrice 0 (length (car (heights finspace)))) differentials)
...
(DEFMETHOD H-REGULAR-DIF ((topogenous matrice))
  (let* ((differentials '())
         ...
         (stong-heights (heights stong)))
    (declare (type list differentials))
    (push (creer-matrice 0 (length (car stong-heights))) differentials)
    (do ((dim_Cn-2 NIL (length (car heights)))
         (heights stong-heights (cdr heights)))
        ((endp (cdr heights)))
      (let* ((Cn-1 (car heights))
             (Cn (cadr heights))
             (dim_n (length Cn))
...
(DEFMETHOD EXPLICIT-H-REGULAR-DIF ((topogenous matrice))
  #| Computes the list of groups and differentials ((C0 C1 ... CN) (D0 D1 ... Dn))
of the explicit-h-regular-chain complex of 'topogenous' where n = height of 'topogenous' |#
  (let* ((groups NIL)
         (differentials NIL)
         (stong (stongmat topogenous))
         (edges (binarymatrice-to-ubasis (nilpot stong)))
                                 ...
                                 (do ((ic 1 (1+ ic)))
                                   ...
                                   (let* ((pseudo NIL)
                                     ...
                                     (do ((ker ker (cdr ker)))
                                         ((endp ker))
                                       (let* ((ilig (caar ker))
                                              (updt (nth (1- ilig) old-group)))
                                         (loop for z in (reverse updt)
                                               do (push (reverse (cons initial (reverse (cons (* (cadar ker) (car z)) (cdr z))))) pseudo))
                                       (insert-term rslt ilig ic (cadar ker))))
                                     (push pseudo rslt2)))
                                 (push (reverse rslt2) groups)
                                 rslt)
                               differentials)))))))
...
(h-regular-dif (bar-subdivision (randomtop 6 .5)))
(explicit-h-regular-dif (bar-subdivision (random-finite-space 6 .5)))
(explicit-h-regular-dif (bar-subdivision (randomtop 6 .5)))
...
(DEFMETHOD H-REGULAR-HOMOLOGY ((topogenous matrice) &rest rest)
  #| Compute the homology of 'topogenous' directly from the list (H-REGULAR-DIF 'topogenous') |#
  #| rest must be: NIL, an integer number or two integer numbers |#
  (let* ((heights (heights (stongmat topogenous)))
         (size (1- (length heights)))
         (range '()))
    (case (length rest)
      (0 (setf range (<a-b> 0 size)))
      (1 (if (> (car rest) size)
      ...
      (2 (if (> (car rest) size)
             (return-from H-REGULAR-HOMOLOGY (progn (format t "~3%Homology in dimensions in ~D :~%" (<a-b> (first rest) (second rest)))
                                               ...
                                               (done)))
           (setf range (<a-b> (first rest) (second rest))))))
    (let ((list (h-regular-dif topogenous)))
      (dolist (dim range)
        (if (> dim size)
            (progn (format t "~3%Homology in dimension ~D :~%" dim)
              (terpri) (terpri)
...
#|
 ...
 (h-regular-homology (top example) 1)
|#
...
(DEFMETHOD CHCM-H-REGULAR ((topogenous matrice))
...
(DEFMETHOD EXPLICIT-CHCM-H-REGULAR ((topogenous matrice))
  (let ((finspace (build-finite-space :top topogenous)))
